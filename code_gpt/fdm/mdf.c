...
void mdf_heat(double ***  __restrict__ u0, 
    ...
    unsigned int start = (rank * npZ) / size;
    unsigned int end = ((rank + 1) * npZ) / size;
    ...
    while (continued && iteration <= maxIterations){
        ...
        int local_continued = continued;
        int global_continued;
        MPI_Allreduce(&local_continued, &global_continued, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
